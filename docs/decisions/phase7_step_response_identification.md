...
可复算脚本为 `scripts/analyze_px4_tau_identification.py`。它在固定
`[0.01,5.00] s`、步长 `0.001 s` 的网格上搜索 \(\tau\)，并对每个候选值闭式求解
\(v(t)=v_\infty+(v_0-v_\infty)e^{-t/\tau}\) 的两个线性系数。机器可读结果写入：

`/Volumes/Expansion/Aegis/aegisair_phase7_step_response_20260817/tau_identification_reanalysis.json`
...
`r1/r2` 的 12 个 identification 拟合值中位数为 `0.729 s`；按写入复算脚本的
“中位数保留一位小数”规则得到名义值 `tau_hat=0.7 s`。全部 18 个拟合值的中位数为
`0.7185 s`。因此 `0.7 s` 是当前栈的代表性经验时间尺度，而不是精确物理常数。
