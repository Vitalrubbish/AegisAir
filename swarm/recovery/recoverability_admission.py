...
class RecoverabilityAdmissionConfig:
    ...
    ring_samples: int = 32
    arc_extra_m: tuple[float, ...] = ()
    arc_segments: tuple[int, ...] = (5, 8)
    ...
    stabilization_wait_s: float = 0.0
    stabilization_speed_margin_mps: float = 0.05
    stabilization_consecutive_samples: int = 3
    ...
    def __post_init__(self) -> None:
        ...
        if any(value <= 0.0 for value in self.arc_extra_m) or any(value < 2 for value in self.arc_segments):
            raise ValueError("弧线候选半径和分段数无效")
        if self.arc_extra_m and not self.arc_segments:
            raise ValueError("启用弧线候选时必须提供分段数")
        ...
        if self.stabilization_wait_s < 0.0 or not 0.0 <= self.stabilization_speed_margin_mps < self.healthy_velocity_limit_mps:
            raise ValueError("稳定等待或速度裕量无效")
        if self.stabilization_consecutive_samples < 1:
            raise ValueError("稳定连续采样数必须为正")
...
class RecoverabilityAdmissionCoordinator:
    ...
    def __init__(self, config: RecoverabilityAdmissionConfig | None = None, *, geometry_only: bool = False) -> None:
        ...
        self.stabilization_started_step: int | None = None
        self.stabilization_stable_samples = 0
        self.stabilization_wait_steps = 0
        self.orphan_goal: np.ndarray | None = None
    ...
    def _candidate_routes(
        ...
        # Optional multi-waypoint arcs cover paths a single detour point cannot
        # represent. The legacy candidate set is unchanged when arc_extra_m is empty.
        start_angle = math.atan2(start[1] - anchor[1], start[0] - anchor[0])
        goal_angle = math.atan2(goal[1] - anchor[1], goal[0] - anchor[0])
        for extra in self.config.arc_extra_m:
            radius = self.config.effective_clearance_m + extra
            for segments in self.config.arc_segments:
                for direction in (-1, 1):
                    angular_distance = (
                        (goal_angle - start_angle) % (2.0 * math.pi)
                        if direction == 1
                        else -((start_angle - goal_angle) % (2.0 * math.pi))
                    )
                    waypoints = [
                        anchor + radius * np.asarray((math.cos(angle), math.sin(angle)))
                        for index in range(1, segments)
                        for angle in (start_angle + angular_distance * index / segments,)
                    ]
                    if all(self._inside_arena(point) for point in waypoints):
                        routes.append([start, *waypoints, goal])
        return routes
    ...
    def _admit(
        ...
        *,
        ...
        if self.geometry_only:
            # 共用候选/几何/初速度上界，仅移除动态 rollout；不伪造预测指标。
            if not geometric_candidates:
                self.state = "rejected_hold"
                self.rejection_count += 1
                self.rejection_reason = "no_geometrically_recoverable_candidate"
                return
            _, clearance, selected = min(geometric_candidates, key=lambda item: (item[0], -item[1]))
            self.geometric_min_clearance_m = clearance
            self.route = [(float(point[0]), float(point[1]), float(altitude)) for point in selected[1:]]
            self.route_index = 0
            self.state = "admitted"
    ...
    def step(
        ...
        *,
        ...
        if mission_change is not None and self.state == "idle":
            ...
            self.orphan_goal = np.asarray(base_goals[failed][:2], dtype=np.float64)
            initial_velocity = np.asarray(
                healthy_snapshot.velocity[:2]
                if healthy_snapshot.velocity is not None
                else (0.0, 0.0), dtype=np.float64,
            )
            if self.config.stabilization_wait_s > 0.0 and np.any(
                np.abs(initial_velocity) > self.config.healthy_velocity_limit_mps + 1e-9
            ):
                self.state = "stabilizing"
                self.stabilization_started_step = step
            else:
                self._decide_from_snapshots(snapshots)

        if self.state == "stabilizing":
            assert self.stabilization_started_step is not None and self.healthy_drone is not None
            if step <= self.stabilization_started_step:
                return self._hold_override()
            self.stabilization_wait_steps = step - self.stabilization_started_step
            observed_velocity = snapshots[self.healthy_drone].velocity
            velocity = np.asarray(observed_velocity[:2], dtype=np.float64) if observed_velocity is not None else None
            stable_limit = self.config.healthy_velocity_limit_mps - self.config.stabilization_speed_margin_mps
            self.stabilization_stable_samples = (
                self.stabilization_stable_samples + 1
                if velocity is not None and np.all(np.isfinite(velocity)) and np.all(np.abs(velocity) <= stable_limit) else 0
            )
            if self.stabilization_stable_samples >= self.config.stabilization_consecutive_samples:
                self._decide_from_snapshots(snapshots)
            elif self.stabilization_wait_steps >= int(math.ceil(self.config.stabilization_wait_s / self.config.dt_s)):
                self.state = "rejected_hold"
                self.rejection_count += 1
                self.rejection_reason = "stabilization_timeout"

        result = RecoveryOverrides()
        if self.healthy_drone is None:
            return result
        if self.state in {"stabilizing", "rejected_hold"}:
            return self._hold_override()
    ...
    def _hold_override(self) -> RecoveryOverrides:
        assert self.healthy_drone is not None and self.hold_goal is not None
        result = RecoveryOverrides()
        result.goal_override[self.healthy_drone] = self.hold_goal
        result.velocity_scale[self.healthy_drone] = 0.0
        return result

    def _decide_from_snapshots(self, snapshots: Mapping[int, DroneSnapshot]) -> None:
        assert self.healthy_drone is not None and self.failed_drone is not None and self.orphan_goal is not None
        healthy_snapshot = snapshots[self.healthy_drone]
        ...
        self._admit(
            ...
            initial_velocity=np.asarray(
                healthy_snapshot.velocity[:2]
                if healthy_snapshot.velocity is not None else (0.0, 0.0), dtype=np.float64,
            ),
            goal=self.orphan_goal,
            failed_trajectory=self._failed_trajectory(snapshots[self.failed_drone]),
    ...
    def summary(self) -> dict[str, object]:
        return {
            ...
            "stabilization_wait_steps": self.stabilization_wait_steps,
            "stabilization_stable_samples": self.stabilization_stable_samples,
            "stabilization_timeout_s": self.config.stabilization_wait_s,
        }
