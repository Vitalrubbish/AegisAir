...
def published_plan_violation(result, accelerations, *, positions, velocities,
                             safe_distances, config, fixed_accelerations=None):
    """只替换原计划首步，核验发布输入是否保持该计划的约束；不重新优化。"""
    if not result.feasible:
        return float('inf')
    if result.plan is None or result.slacks is None:
        return None
    ids=sorted(positions)
    if set(accelerations)!=set(ids):
        return None
    plan=result.plan.copy()
    for i,drone in enumerate(ids):
        plan[0,i]=accelerations[drone]
    if not np.all(np.isfinite(plan)):
        return float('inf')
    p,v=_predict(plan,positions=positions,velocities=velocities,drone_ids=ids,config=config)
    fixed=fixed_accelerations or {}
    violations=[0.0, float(-np.min(result.slacks))]
    for i,drone in enumerate(ids):
        if drone in fixed:
            violations.append(float(np.max(np.abs(plan[:,i]-fixed[drone]))))
        else:
            violations.append(float(np.max(np.abs(plan[:,i]))-config.a_max))
        violations.extend([float(np.max(np.abs(p[drone]))-config.position_bound_m),
                           float(np.max(np.abs(v[drone]))-config.velocity_bound_mps),
                           float(np.max(np.abs(v[drone][-1]))-config.terminal_velocity_tolerance_mps)])
    for (i,j),distance in safe_distances.items():
        separation=np.sum((p[i][:-1]-p[j][:-1])**2,axis=1)
        violations.append(float(np.max(distance**2-separation-result.slacks)))
        violations.append(float((distance+config.terminal_buffer_m)**2-np.sum((p[i][-1]-p[j][-1])**2)))
