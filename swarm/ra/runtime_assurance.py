...
from swarm.ra.pcbf import PCBFConfig, solve_pcbf, published_plan_violation
...
class RuntimeAssurance:
    ...
    def __init__(
        ...
        hocbf_recovery_distance_threshold_m: float | None = None,
        hocbf_recovery_ttc_threshold_s: float | None = None,
        ...
        for value in (hocbf_recovery_distance_threshold_m, hocbf_recovery_ttc_threshold_s):
            if value is not None and (not np.isfinite(value) or value <= 0):
                raise ValueError("距离/TTC 触发阈值必须有限且为正")
        if sum(value is not None for value in (
            hocbf_recovery_distance_threshold_m, hocbf_recovery_ttc_threshold_s,
            hocbf_recovery_reserve_threshold,
        )) > 1:
            raise ValueError("距离、TTC、reserve 阈值对照不可混用")
        self.hocbf_recovery_distance_threshold_m = hocbf_recovery_distance_threshold_m
        self.hocbf_recovery_ttc_threshold_s = hocbf_recovery_ttc_threshold_s
    ...
    def audit_published_accelerations(
        ...
        *,
        ...
        if context is None:
            return None, None
        if context['selected_filter']=='pcbf':
            violation=published_plan_violation(context['result'],accelerations,
                positions=context['positions'],velocities=context['velocities'],
                safe_distances=context['safe_distances'],config=context['config'],
                fixed_accelerations=context['fixed_accelerations'])
            if violation is None:
                return None,None
            return violation <= context['config'].acceptable_tolerance, -float(violation)
        try:
            ...
            elif selected_filter in {"pb_recovery", "pb_cbf"}:
    ...
    def _filter_sampled_data(
        ...
        if self.sampled_data_method == "pcbf":
            ...
            self._published_audit_context = dict(
                selected_filter='pcbf', result=pcbf_result,
                positions={i:v.copy() for i,v in positions.items()},
                velocities={i:v.copy() for i,v in velocities.items()},
                safe_distances={pair:distance+self.sampled_data_boundary_buffer_m
                    for pair,distance in (s_now if self.constraint_boundary=='full' else s_static).items()},
                config=replace(self.pcbf_config,dt=dt,a_max=self.a_max,
                    control_scale_s=(1.0-float(np.exp(-dt/self.tau_px4)) if self.tau_px4>0 else 1.0)*command_scale),
                fixed_accelerations={i:v.copy() for i,v in fixed_accelerations.items()})
            ...
            self._published_audit_context = {
                "selected_filter": "pb_cbf",
                "positions": {i: value.copy() for i, value in positions.items()},
                "velocities": {i: value.copy() for i, value in velocities.items()},
                "safe_distance": {
                    p: d + self.sampled_data_boundary_buffer_m
                    for p, d in s_static.items()
                },
                "a_max": self.a_max,
                "box_constrained_drones": tuple(
                    i for i in drone_ids if i not in fixed_accelerations
                ),
                "alpha": self.pb_alpha,
    ...
    def _filter_hocbf(
        ...
        if self.hocbf_pb_recovery:
            from swarm.ra.trigger_baselines import proximity_trigger
            proximity_reason = proximity_trigger(
                positions, velocities, d_safe_map,
                distance_m=self.hocbf_recovery_distance_threshold_m,
                ttc_s=self.hocbf_recovery_ttc_threshold_s,
            ) if (self.hocbf_recovery_distance_threshold_m is not None or
                  self.hocbf_recovery_ttc_threshold_s is not None) else None
            ...
            elif proximity_reason is not None:
                recovery_trigger = proximity_reason
