...
class Phase5CommandEncodingTest(unittest.TestCase):
    ...
    def test_solver_fallback_is_visible_without_claiming_publication_mismatch(self) -> None:
        result = SimpleNamespace(feasible=False, primary_feasible=False)
        reason = _solver_fallback_reason(result)
        self.assertEqual(reason, "RA_QP_INFEASIBLE_FALLBACK")
        command = build_phase5_velocity_command(
            drone=2, safe_velocity=(0.0, 0.0), vertical_velocity=0.0, timestamp_ms=1000)
        audit = _audit_published_command(
            solver_selected_velocity=(0.0, 0.0), published_command=command,
            reference_velocity=(1.0, 0.0), command_scale_s=0.7, fallback_reason=reason)
        self.assertTrue(audit['published_command_matches_selected'])
        self.assertEqual(audit['fallback_reason'], reason)
        self.assertIsNone(_solver_fallback_reason(SimpleNamespace(feasible=True, primary_feasible=False)))
        self.assertIsNone(_solver_fallback_reason(SimpleNamespace(feasible=None)))
