...
class MeasurementTimestampTest(unittest.TestCase):
    def test_republication_does_not_make_cached_position_younger(self) -> None:
        stamps = [_measurement_timestamp_ms(now_ms=t, position_arrival_ms=1000,
                  status_arrival_ms=t, stale_after_s=1.5) for t in (1010,1060,1110)]
        self.assertEqual(stamps, [1000,1000,1000])
        self.assertEqual([t-stamp for t,stamp in zip((1010,1060,1110),stamps)], [10,60,110])
