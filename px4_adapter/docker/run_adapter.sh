...
for instance in $ADAPTER_INSTANCES; do
    ...
    adapter_pids+=("$!")
    # PX4 instances share one uXRCE agent.  Starting all ROS subscribers in
    # the same scheduler slice can lose initial discovery on macOS Docker;
    # register each instance before creating the next one.
    sleep "${ADAPTER_STARTUP_STAGGER_S:-1}"
done
