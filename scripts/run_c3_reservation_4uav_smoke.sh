#!/usr/bin/env bash
# 四机 C3-Admission/R3-Reservation 新协议 calibration。
set -uo pipefail
...
RUNNER="${C3_RUNNER:-marllib/run_c3_reservation_gazebo.py}"
CONDITION="${C3_CONDITION:-}"
TRIAL_SEED="${C3_TRIAL_SEED:-}"
GAZEBO_SEED="${C3_GAZEBO_SEED_OVERRIDE:-9521}"
HEARTBEAT="$GATE0/scripts/gcs_heartbeat.py"
TELEMETRY_WAIT="$REPO/scripts/wait_for_px4_telemetry.py"
adapter_name="aegisair-c3-reservation-adapters"
heartbeat_pids=()
state_dir=""
launch_pid=""
...
stop_stack() {
  ...
  if [[ -n "$state_dir" && -d "$state_dir" ]]; then
    ...
    # Reap the launcher before testing command-line identity.  Otherwise dead
    # PX4 children can remain as zombies and make the next isolated launch look
    # like an overlapping live instance.
...
for port in 18572 18573 18574 18575; do
  "$PY" "$HEARTBEAT" --port "$port" --duration-s 14400 >"$OUT/gcs${port}.log" 2>&1 &
  heartbeat_pids+=("$!")
done
...
if ! "$PY" "$TELEMETRY_WAIT" --drone-ids 2 3 4 5 --timeout-s 90; then
  touch "$OUT/invalid_telemetry_readiness"
  exit 1
fi

(
  cd "$REPO"
  runner_args=(--manifest "$MANIFEST" --output "$OUT/calibration")
  if [[ -n "$CONDITION" ]]; then
    runner_args+=(--condition "$CONDITION")
  fi
  if [[ -n "$TRIAL_SEED" ]]; then
    runner_args+=(--seed "$TRIAL_SEED")
  fi
  conda run -n eai-swarm python "$RUNNER" "${runner_args[@]}"
) | tee "$OUT/runner.log"
