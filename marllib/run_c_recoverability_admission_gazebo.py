...
IMPLEMENTATION_VERSION_V5 = "dynamic_admission_v5_stabilize_reevaluate"
IMPLEMENTATION_VERSION_V6 = "dynamic_admission_v6_arc_candidates_probe"
COMPARISON_PROTOCOLS = {
    "aegisair-admission-comparison-development-v1",
    "aegisair-admission-comparison-sealed-v1",
...
def _admission_config(
    ...
    *,
    ...
    return RecoverabilityAdmissionConfig(
        ...
        arc_extra_m=tuple(float(value) for value in config.get("arc_extra_m", ())),
        arc_segments=tuple(int(value) for value in config.get("arc_segments", (5, 8))),
        ...
        stabilization_wait_s=float(config.get("stabilization_wait_s", 0.0)),
        stabilization_speed_margin_mps=float(config.get("stabilization_speed_margin_mps", 0.05)),
        stabilization_consecutive_samples=int(config.get("stabilization_consecutive_samples", 3)),
    )
...
def main() -> int:
    ...
    parser.add_argument("--condition", choices=sorted(CONDITIONS | {"GEOMETRY_ONLY_ADMISSION_RA"}), required=True)
    args = parser.parse_args()
    manifest = json.loads(args.manifest.read_text(encoding="utf-8"))
    if manifest.get("protocol_id") not in PROTOCOL_IDS | COMPARISON_PROTOCOLS:
    ...
    if manifest.get("implementation_version") != expected_version:
        parser.error(
            ...
            f"{expected_version!r}，不得混合新旧接纳协议"
        )
    if args.condition == "GEOMETRY_ONLY_ADMISSION_RA" and manifest.get("protocol_id") not in COMPARISON_PROTOCOLS:
        parser.error("几何对照只允许使用新对照协议，不修改旧 v4 条件集")
    ...
    if args.condition == "IMMEDIATE_COMMIT_RA":
    ...
    elif args.condition in {"RECOVERABILITY_ADMISSION_RA", "GEOMETRY_ONLY_ADMISSION_RA"}:
        coordinator = RecoverabilityAdmissionCoordinator(
            ...
            geometry_only=args.condition == "GEOMETRY_ONLY_ADMISSION_RA",
        )
    else:
