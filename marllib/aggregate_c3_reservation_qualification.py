#!/usr/bin/env python3
"""汇总四机 R2/R3 qualification，并区分无效启动与有效算法失败。"""
...
def collect(manifest_path: Path, root: Path) -> dict[str, Any]:
    ...
    r3_rows = [row for row in rows if row["condition"] == "R3_C3_RESERVATION"]
    any_r3_failure = any(row["r3_qualification_go"] is False for row in r3_rows)
    expected = len(manifest["qualification_seeds"]) * len(CONDITIONS)
    complete = len(rows) == expected
    qualification_go = bool(
        complete
        and len(r3_rows) == len(manifest["qualification_seeds"])
        and all(row["r3_qualification_go"] for row in r3_rows)
    )
    decision = "NO_GO" if any_r3_failure else "GO" if qualification_go else "INCOMPLETE"
    return {
        ...
        "r3_valid_trials": len(r3_rows),
        "r3_passed_trials": sum(bool(row["r3_qualification_go"]) for row in r3_rows),
