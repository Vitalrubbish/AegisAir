...
def _solver_fallback_reason(result: object) -> str | None:
    """Expose an infeasible solver fallback even when publication is faithful."""
    # 此字段只描述已有本地轨迹，不新增或改变飞控消息。
    # primary_feasible=False 而 PB recovery feasible=True 不属于输出回退。
    if getattr(result, "feasible", None) is False:
        return "RA_QP_INFEASIBLE_FALLBACK"
    return None
    ...
    *,
    ...
    def make_ra() -> RuntimeAssurance:
        return RuntimeAssurance(
            ...
            hocbf_recovery_distance_threshold_m=hocbf_recovery_distance_threshold_m,
            hocbf_recovery_ttc_threshold_s=hocbf_recovery_ttc_threshold_s,
    ...
    try:
        ...
        while step < max_steps:
            ...
            for i in drone_ids:
                ...
                fallback_reason = _solver_fallback_reason(results[i])
                if i in failed or i in overrides.aborted:
