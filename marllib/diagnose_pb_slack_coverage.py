...
def traces(root):
    ...
    return [(p,[json.loads(l) for l in p.read_text(encoding="utf-8").splitlines()]) for p in paths]
...
def main():
    ...
    # 先检查原始挂载；不把数据写到误建的本地 /Volumes 目录。
    if not Path('/Volumes/Expansion').is_mount():
        raise SystemExit('Expansion 未挂载；仅可继续源码审计，不产生数据结果')
    if not args.out.resolve().is_relative_to(Path('/Volumes/Expansion').resolve()):
        raise SystemExit('诊断数据必须写入移动硬盘')
    if args.out.exists():
        raise SystemExit('输出已存在，拒绝覆盖')
    old=json.loads(args.archived_audit.read_text(encoding="utf-8"))
    ...
    for entry in old['evaluation']:
        ...
        trace=[json.loads(l) for l in path.read_text(encoding="utf-8").splitlines()]
        rows=[];age=0
