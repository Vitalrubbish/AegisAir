#!/usr/bin/env python3
"""运行一个全新 SITL 的四机 R3/R4 C3-STR qualification 条件。"""
...
from marllib.run_c3_reservation_gazebo import (
    _audit as _r3_audit,
    _coordinators,
    _sha256,
)
from marllib.run_c3_space_time_reservation_gazebo import (
    _audit as _r4_audit,
    _coordinator as _r4_coordinator,
)


PROTOCOL_ID = "aegisair-c3-str-4uav-qualification-v1"
CONDITIONS = ("R3_C3_RESERVATION", "R4_C3_STR")
...
def validate_manifest(manifest: dict[str, Any]) -> None:
    ...
    for seed in seeds:
        ...
        if sorted(order) != sorted(CONDITIONS):
            raise ValueError(f"seed {seed} 必须且只能各运行一次 R3/R4")
    ...
    schedule_config = manifest.get("space_time_reservation", {})
    derived = (
        float(schedule_config.get("staging_window_s", 0.0))
        + 4.0 * float(schedule_config.get("service_window_s", 0.0))
        + 2.0 * float(schedule_config.get("final_return_window_s", 0.0))
    )
    if derived != 52.0:
        raise ValueError("C3-STR qualification 必须保持冻结的 52 s 预约构造")
    if int(manifest.get("max_invalid_attempts_per_condition", 0)) < 1:
        raise ValueError("必须冻结无效启动的最大重试次数")


def _completion_step(trajectory: Path, condition: str) -> int | None:
    key = (
        "coordination_reservation"
        if condition == "R3_C3_RESERVATION"
        else "coordination_space_time_reservation"
    )
...
def _r4_go(row: dict[str, Any], max_steps: int) -> bool:
    state = row["c3_space_time_reservation_summary"] or {}
    audit = row["audit"]
    ...
    return bool(
        ...
        and any(len(group) > 1 for group in state.get("final_return_groups", []))
...
def main() -> int:
    ...
    if args.condition == "R3_C3_RESERVATION":
        ...
        space_time = _r4_coordinator(manifest, drone_ids, starts, goals)
    ...
    run = run_mqtt_loop(
        ...
        space_time_reservation_coordinator=space_time,
        land_at_end=True,
        **_method_kwargs(
            manifest["method"],
    ...
    audit = (
        _r3_audit(trajectory, args.condition)
        if args.condition == "R3_C3_RESERVATION"
        else _r4_audit(trajectory, drone_ids)
    )
    row = {
        "seed": args.seed,
        ...
        "completion_step": _completion_step(trajectory, args.condition),
        ...
        "c3_space_time_reservation_summary": run["c3_space_time_reservation"],
        "audit": audit,
    ...
    row["r4_qualification_go"] = (
        _r4_go(row, int(manifest["max_steps"]))
        if args.condition == "R4_C3_STR"
        else None
    )
    summary = {
